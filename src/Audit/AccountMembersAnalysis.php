...
class AccountMembersAnalysis extends AbstractAnalysis {
  ...
  public function gather(Sandbox $sandbox)
  {
    ...
    do {
      $response = $this->api()->request('GET', "accounts/{$zone['account']['id']}/members", ['query' => [
        'per_page' => 50,
        'page' => $page
      ]]);
      $page++;
    ...
    if ($pattern = $sandbox->getParameter('exclude')) {
      $accounts = array_filter($accounts, function ($account) use ($pattern) {
        return strpos($account['user']['email'], $pattern) === FALSE;
      });
    }

    $sandbox->setParameter('accounts', array_values($accounts));
  }
}
